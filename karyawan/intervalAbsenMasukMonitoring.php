...
$result = mysqli_query($conn, "SELECT * FROM a_masuk_karyawan am JOIN tb_karyawan k ON am . id_karyawan = k . id_karyawan JOIN tb_jabatan tj ON k . id_jabatan = tj . id_jabatan WHERE am . m_bulan_tahun = '$m_bulan_tahun' ORDER BY k . nama ASC");
      ...
      <?php
         for ($i = 1; $i <= $jml_hari; $i++) {
            ...
            if (!empty($a_masuk_karyawan[$i])) {
               ...
               echo '<td class="cursor-pointer info-masuk" data-token_masuk="' . $a_masuk_karyawan[$i] . '" data-id_siswa="' . $a_masuk_karyawan['id_siswa'] . '">';
